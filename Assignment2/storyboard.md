# UX Design

## Mindmaps

### Sebastien:
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/42de82dd3ebb20699de54e740143693c888060df/Assignment2/UX%20Design%20Images/mindmapSD.jpg" width = "600">
</p>

Sebastien - I was trying to just show how I want everything to be conntected and have as many user choices be available on each page as possible. 

Ryan - I like the circular flow of this mindmap, which points to the map as being the center of the design.

Ainsleigh - I like how the design of this webmap shows the nonlinear path users take when navigating a website.

### Ainsleigh:
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/Ainsleigh-MindMap.jpg" width = "600">
</p>

Sebastien - Ainsleigh modeled the website a little more than I did but I like what she did. Her sketch looks good and lays out a good baseline for what the site should look like.

Ryan - This presented a great start to the design process for the final site. Ainsleigh was able to present more design-centered elements that will ensure that our final site looks good and will present an effective user experience.

Ainsleigh - I tried to think of new ways that the user could interact with a site, such as horizontal scrolling. I think a design like this might make it more accessible to people who want to focus on one element of the project (ex/ visualizations).

### Ryan: 
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/Ryan-Mindmap.JPG" width = "600">
</p>

Sebastien - Ryan also mapped out the layout of the website and I like the way he showed the flow from top to bottom.

Ryan - I was mostly focusing on incorporating narrative elements into our site, so that it is not overwhelemed by data and can instead tell a complete story.

Ainsleigh - I like how this design has a solid focal point and flow, this makes navigating a complex subject have a lighter cognitive load on the user.

## Crazy Eights

### Sebastien:
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/crazy8sd.jpg" width = "600">
</p>

Sebastien - I just gave examples of what I wanted each site to look like with very basic detail and descriptions. 

Ryan - These ideas present the essentials of what our project will need to successully present our data and findings.

### Ainsleigh:
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/Ainsleigh-CrazyEights.png" width = "600">
</p>

Sebastien -  I liked the way Ainsleigh made each of her slides look. I like the visual element.

Ryan - One thing I really like about this design is the idea for horizontal scrolling across our site. I think it presents an opportunity to make our project more unique and useful to the user.

### Ryan:
<p align="center">
<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/Ryan-CrazyEights2.JPG" width = "600">
</p>

Sebastien - Ryan took a different approach which I appreciate. He focused on design features that he wants to include.  

Ryan - My main focus was to come up with some ideas that would make our site stand out and also present the user with a more unique and enriching experience.

## Storyboard:

<img src = "https://github.com/ryanorlando/DH151-Project/blob/33a6ee928a2b10b7264e9d390dd3aaf1d9ece3df/Assignment2/UX%20Design%20Images/StoryBoard-wireframe.png">




